...
function AddForm() {
  ...
  async function handleSave() {
    if (!description || !amount || !category) { setError("Preencha todos os campos."); return; }
    ...
    <DashboardWrapper>
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          <Link href="/" className="w-10 h-10 bg-brand-muted rounded-xl flex items-center justify-center"><ArrowLeft size={18}/></Link>
        ...
        <div className="space-y-3">
          <input className="input-field" placeholder="Descrição (ex: Mercado, Salário...)" value={description} onChange={e => setDescription(e.target.value)} />
          <div>
...
export default function AdicionarPage() {
  return (
    <Suspense fallback={<DashboardWrapper><div className="flex items-center justify-center h-64"><div className="w-8 h-8 border-2 border-brand-green border-t-transparent rounded-full animate-spin"/></div></DashboardWrapper>}>
      <AddForm />
    </Suspense>
  );
}
